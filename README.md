...
2. 페이지의 가로 스크롤 영역이 아닌 곳에서 두 손가락을 오른쪽으로 짧게 당기고 놓으면 한 단계 **뒤로가기**, 반대쪽으로 하면 한 단계 **앞으로가기**가 실행됩니다.
3. 같은 방향으로 설정한 시간 동안 계속 당기면 해당 방향의 방문 기록 목록이 열립니다. 기본값은 0.5초입니다.
...
7. 메뉴는 바깥을 클릭하거나 `Esc`를 눌러 닫을 수도 있습니다. 키보드 `↑`·`↓`와 `Enter`로도 조작할 수 있습니다.

macOS의 스크롤 방향 설정에 따라 뒤로가기와 앞으로가기가 반대로 인식되면 Chrome 툴바의 확장 아이콘을 눌러 **뒤로가기 방향**을 바꾸세요. 같은 팝업에서 기능을 끄거나 기록 메뉴 대기 시간을 0.3·0.5·0.7·1.0초로 조절할 수 있습니다.

## 구현 방식

- 콘텐츠 스크립트가 취소 가능한 가로 `wheel` 이벤트의 지속 시간으로 동작을 구분합니다. 설정한 대기 시간 전에 입력이 끝나면 한 단계 이동하고, 입력이 그 시간 동안 이어지면 메뉴를 엽니다.
...
- 이동할 이전·다음 기록이 없는 짧은 제스처는 오류로 표시하지 않고 조용히 무시합니다.
