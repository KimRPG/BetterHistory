...
2. 페이지의 가로 스크롤 영역이 아닌 곳에서 두 손가락을 오른쪽으로 짧게 당기면 **뒤로가기 기록**, 반대쪽으로 하면 **앞으로가기 기록** 목록이 열립니다.
3. 같은 방향의 입력이 0.5초 이상 이어지면 해당 방향으로 한 단계 이동합니다.
...
- 콘텐츠 스크립트가 취소 가능한 가로 `wheel` 이벤트의 지속 시간으로 동작을 구분합니다. 0.5초 전에 입력이 끝나면 메뉴를 열고, 0.5초 동안 이어지면 한 단계 이동합니다.
