...
3. 같은 방향으로 설정한 시간 이상 계속 당기면 해당 방향으로 한 단계 이동합니다. 기본값은 0.5초입니다.
   단, 테스트용 **0.1초 또는 0.2초**를 선택하면 동작이 반대로 바뀌어 짧게 당길 때 한 단계 이동하고 설정 시간 이상 유지할 때 기록 메뉴가 열립니다.
...
- 콘텐츠 스크립트가 취소 가능한 가로 `wheel` 이벤트의 지속 시간으로 동작을 구분합니다. 0.3·0.5초 설정은 짧게 입력하면 메뉴, 길게 입력하면 한 단계 이동이며, 테스트용 0.1·0.2초 설정은 두 동작의 순서를 뒤집습니다.
- 가로 제스처의 이동량에 맞춰 페이지 본문을 최대 72px까지 움직이고, 제스처가 끝나면 0.22초 동안 원위치로 복귀시킵니다. 메뉴 UI는 페이지 이동의 영향을 받지 않습니다.
- Navigation API가 해당 방향에 이동 가능한 페이지가 없다고 알려 주면 페이지 이동 모션을 생략합니다. 보안상 확인할 수 없는 교차 출처 기록은 모션 없이 이동 기능만 동작할 수 있습니다.
...
- 이동할 이전·다음 기록이 없는 한 단계 제스처는 오류로 표시하지 않고 조용히 무시합니다.
...
- 브라우저가 웹페이지에 정확한 손가락 떼기 이벤트를 제공하지 않아, 짧은 제스처는 마지막 입력 후 약 0.19초, 메뉴 내 선택은 약 0.3초가 지난 시점을 손을 뗀 것으로 처리합니다.
