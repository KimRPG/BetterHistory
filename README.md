...
4. 열린 메뉴에서 항목을 클릭하거나 키보드 `↑`·`↓`와 `Enter`로 이동할 페이지를 선택합니다.
5. 열린 메뉴를 두 손가락으로 오른쪽으로 살짝 밀고 손을 떼면 닫힙니다.
6. 메뉴는 바깥을 클릭하거나 `Esc`를 눌러 닫을 수도 있습니다.

macOS의 스크롤 방향 설정에 따라 뒤로가기와 앞으로가기가 반대로 인식되면 Chrome 툴바의 확장 아이콘을 눌러 **뒤로가기 방향**을 바꾸세요. 같은 팝업에서 기능을 끄거나 길게 당기기 기준을 0.3·0.5·0.7·1.0초로 조절할 수 있습니다.
