...
3. 같은 방향으로 설정한 시간 이상 계속 당기면 해당 방향으로 한 단계 이동합니다. 기본값은 0.5초입니다.
   단, 테스트용 **0.1초**를 선택하면 동작이 반대로 바뀌어 짧게 당길 때 한 단계 이동하고 0.1초 이상 유지할 때 기록 메뉴가 열립니다.
...
- 콘텐츠 스크립트가 취소 가능한 가로 `wheel` 이벤트의 지속 시간으로 동작을 구분합니다. 일반 설정은 짧게 입력하면 메뉴, 길게 입력하면 한 단계 이동이며, 테스트용 0.1초 설정만 두 동작의 순서를 뒤집습니다.
